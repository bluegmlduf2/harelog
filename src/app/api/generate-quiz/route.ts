...
// AI를 통해 퀴즈 생성
const generateQuizWithAI = async (difficulty: string, language: string, userLanguage: string): Promise<Quiz> => {
    const languageMap = {
        'ko': '한국어',
        'en': 'English', 
        'ja': '日本語'
    ...
    const difficultyMap = {
        'easy': userLanguage === 'ko' ? '초급' : userLanguage === 'en' ? 'Beginner' : '初級',
        'medium': userLanguage === 'ko' ? '중급' : userLanguage === 'en' ? 'Intermediate' : '中級', 
        'hard': userLanguage === 'ko' ? '고급' : userLanguage === 'en' ? 'Advanced' : '上級'
    };

    const prompt = userLanguage === 'ko' 
        ? `${difficultyMap[difficulty as keyof typeof difficultyMap]} 수준의 ${language} 프로그래밍 언어로 된 코딩 퀴즈를 생성해주세요.
...
반드시 JSON 형식으로만 응답해주세요.`
        : userLanguage === 'en'
        ? `Generate a ${difficultyMap[difficulty as keyof typeof difficultyMap]} level coding quiz in ${language} programming language.
...
Please respond only in JSON format.`
        : `${difficultyMap[difficulty as keyof typeof difficultyMap]}レベルの${language}プログラミング言語でコーディングクイズを生成してください。
...
export async function POST(request: NextRequest) {
    try {
        ...
        // AI를 통해 퀴즈 생성
        const quiz = await generateQuizWithAI(difficulty, language, userLanguage);
        ...
        if (error instanceof Error) {
            ...
            // AI 파싱 에러 처리
            if (errorMessage.includes("AI가 생성한 퀴즈 데이터를 처리할 수 없습니다")) {
                return NextResponse.json(
